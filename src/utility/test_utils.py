...
from src.utility.utils import load_data, load_dataV1, packetloss_threshold, \
    energyconsumption_threshold, utility, set_log_dir, return_next_item, scale_data, get_tts_qs, get_tt_qs
import pandas as pd
...
def evaluate_models(models):
    data = load_dataV1(path=PATH, load_all=False, version='')
        ...
        try:
            ...
            for keys, values in models.items():
                for key, value in values.items():
                    ...
                    features = df[['energyconsumption', 'packetloss', 'latency']].iloc[1:2,:].to_numpy()
                    # predicted_multi = np.argsort(
...
def plot_quality_properties(plot_type=PLOT_TYPE, for_type=PLOT_TYPE, path=PATH):
    data, _ = load_dataV1(path=path, load_all=False, shuffle=SHUFFLE,
