...
def main():
    ...
    # Input parameters
    env_name = st.selectbox("Environment Name", [
                            'DeltaIoTv1', 'DeltaIoTv2', 'BDBC_AllNumeric'])
    ...
    if env_name in ['DeltaIoTv1', 'DeltaIoTv2']:
        ...
        if strategy_type == 'multi':
            additional_params['energy_coef'] = st.number_input(
                "Energy Coefficient", min_value=0.0, max_value=1.0, value=0.1)
            additional_params['packet_coef'] = st.number_input(
                "Packet Coefficient", min_value=0.0, max_value=1.0, value=0.8)
            additional_params['latency_coef'] = st.number_input(
                "Latency Coefficient", min_value=0.0, max_value=1.0, value=0.8)
    ...
    algo_name = st.selectbox(
        "Algorithm Name", ['DQN', 'PPO', 'A2C', 'HER_DQN'])
    ...
    exploration_fraction = st.text_input(
        "Exploration Fraction (for DQN)", "0.1,0.2,0.4,0.6")
    ...
    max_episode_steps = st.number_input(
        "Max Episode Steps", additional_params['max_episode_steps'])
    ...
    if execution_mode == 'Parallel':
        num_processes = st.number_input(
            "Number of Processes", min_value=1, value=4)
    ...
    if st.button("Train Model"):
        try:
            ...
            stable_dqn(env_name, algo_name, policy, lr, exploration_fraction, gamma, batch_size, total_timesteps, chkpt_dir,
                       log_path, warmup_count, eps_min, epsilon, num_pulls, additional_params, execution_mode, num_processes)
...
def setup_env(env_name, additional_params):
    ...
    if env_name in ['DeltaIoTv1', 'DeltaIoTv2']:
        env = Monitor(TimeLimit(DeltaIotEnv(
            data_dir=glob.glob(os.path.join(
                f'data/{env_name}/train', "*.json")),
...
def stable_dqn(env_name, algo_name, policy, lr, exploration_fraction, gamma, batch_size, total_timesteps, chkpt_dir, log_path, warmup_count, eps_min, epsilon, num_pulls, additional_params, execution_mode, num_processes=None):
    ...
    if execution_mode == 'Parallel':
        with ProcessPoolExecutor(max_workers=num_processes) as executor:
            for lr in lrs:
                for exploration_fraction in exploration_fractions:
                    tasks.append(executor.submit(train_model, env_name, algo_name, policy, float(lr), float(exploration_fraction), gamma,
                                 batch_size, total_timesteps, chkpt_dir, log_path, warmup_count, eps_min, epsilon, num_pulls, additional_params))
        ...
        for lr in lrs:
            for exploration_fraction in exploration_fractions:
                train_model(env_name, algo_name, policy, float(lr), float(exploration_fraction), gamma, batch_size,
                            total_timesteps, chkpt_dir, log_path, warmup_count, eps_min, epsilon, num_pulls, additional_params)
...
def train_model(env_name, algo_name, policy, lr, exploration_fraction, gamma, batch_size, total_timesteps, chkpt_dir, log_path, warmup_count, eps_min, epsilon, num_pulls, additional_params):
    ...
    log_path_base = os.path.join(log_path, f"{env_name}-policy={policy}-lr={lr}-batch_size={batch_size}-gamma={
                                 gamma}-total_timesteps={total_timesteps}-exploration_fraction={exploration_fraction}")
    checkpoint_callback = CheckpointCallback(save_freq=1, save_path=chkpt_dir, name_prefix=f"{env_name}-policy={policy}-lr={
                                             lr}-batch_size={batch_size}-gamma={gamma}-total_timesteps={total_timesteps}-exploration_fraction={exploration_fraction}")
    eval_callback = EvalCallback(env, callback_on_new_best=checkpoint_callback, eval_freq=int(
        total_timesteps / 20), verbose=1, n_eval_episodes=5)

    if algo_name == "DQN":
        train_dqn(env, policy, lr, exploration_fraction, gamma, batch_size, total_timesteps, warmup_count, eps_min,
                  epsilon, num_pulls, additional_params.get('setpoint_thresh'), log_path_base, checkpoint_callback, eval_callback)
    elif algo_name == "PPO":
        train_ppo(env, policy, lr, gamma, batch_size, total_timesteps,
                  log_path_base, checkpoint_callback, eval_callback)
    elif algo_name == "A2C":
        train_a2c(env, policy, lr, gamma, total_timesteps,
                  log_path_base, checkpoint_callback, eval_callback)
    elif algo_name == "HER_DQN":
        train_her_dqn(env, policy, lr, exploration_fraction, gamma, batch_size, total_timesteps, warmup_count, eps_min,
                      epsilon, num_pulls, additional_params.get('setpoint_thresh'), log_path_base, checkpoint_callback, eval_callback)
...
def train_dqn(env, policy, lr, exploration_fraction, gamma, batch_size, total_timesteps, warmup_count, eps_min, epsilon, num_pulls, setpoint_thresh, log_path, checkpoint_callback, eval_callback):
    ...
    model.learn(total_timesteps=total_timesteps,
                log_interval=1, callback=eval_callback)
...
def train_her_dqn(env, policy, lr, exploration_fraction, gamma, batch_size, total_timesteps, warmup_count, eps_min, epsilon, num_pulls, setpoint_thresh, log_path, checkpoint_callback, eval_callback):
    # Example strategy (can be 'future', 'final', 'episode')
