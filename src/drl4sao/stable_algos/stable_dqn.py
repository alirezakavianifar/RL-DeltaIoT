...
def stable_dqn(agent_params):
    ...
    log_path = os.path.join(agent_params['log_path'], f"{agent_params['env_name']}_{name}-policy={agent_params['policy']}-lr={agent_params['lr']}-eps_min={agent_params['eps_min']}-batch_size={agent_params['batch_size']}-gamma={agent_params['gamma']}-exploration_fraction={agent_params['exploration_fraction']}-total_timesteps={agent_params['total_timesteps']}")
    checkpoint_callback = CheckpointCallback(save_freq=1, save_path=save_path,
                                             name_prefix=f"{agent_params['env_name']}_{name}-policy={agent_params['policy']}-lr={agent_params['lr']}-eps_min={agent_params['eps_min']}-batch_size={agent_params['batch_size']}-gamma={agent_params['gamma']}-exploration_fraction={agent_params['exploration_fraction']}-total_timesteps={agent_params['total_timesteps']}")
    eval_callback = EvalCallback(
        env, callback_on_new_best=checkpoint_callback, eval_freq=2_000, verbose=1, n_eval_episodes=5)
